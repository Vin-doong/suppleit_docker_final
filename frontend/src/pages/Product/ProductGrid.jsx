...
const ProductGrid = () => {
  ...
  const toggleProductSet = () => {
    ...
    setActiveSet(newSet);
    sessionStorage.setItem('activeSet', JSON.stringify(newSet));
  };
  ...
  return (
    <div className="w-full flex flex-col items-center">
      ...
      <div className="w-full flex justify-center">
        <div className="grid grid-cols-3 gap-x-4 gap-y-8 max-w-4xl w-full">
          {products.map((product, index) => (
            <div
              key={index}
              className="bg-white p-6 rounded-lg shadow-md hover:shadow-lg transition-shadow cursor-pointer text-center min-h-[180px] flex flex-col justify-between w-[300px] min-w-[250px]"  // 고정된 너비 및 최소 너비 설정
              onClick={() => handleCategoryClick(product.title)}
            >
              <i className={`fas fa-${product.icon} text-3xl text-teal-500 mb-4`}></i>
              <h3 className="text-lg font-medium text-gray-800 h-[48px] flex items-center justify-center">
                {product.title}
              </h3>
